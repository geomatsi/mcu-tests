...
mod app {
    ...
    #[task(binds = EXTI1, priority = 2, local = [irq, nfc])]
    fn exti1(cx: exti1::Context) {
        if cx.local.irq.check_interrupt() {
            match cx.local.nfc.reqa() {
                Ok(atqa) => match cx.local.nfc.select(&atqa) {
                    Ok(uid) => rprintln!("NFC: * {:?}", uid.as_bytes()),
                    Err(_) => rprintln!("NFC: failed to read UID"),
                },
                Err(_) => rprintln!("NFC: empty IRQ"),
            }

            cx.local.irq.clear_interrupt_pending_bit();
        } else {
            rprintln!("NFC: unexpected IRQ");
        }
    }
